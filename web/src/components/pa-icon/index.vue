  ...
  <div class="w-full">
    ...
    <a-modal v-model:visible="visible" width="800px" draggable :footer="false">
      ...
      <a-tabs class="tabs">
        ...
        <a-tab-pane key="mine" title="PeanutAdmin">
          <ul class="mine">
            <li
              v-for="icon in paIcons"
              :key="icon"
...
<script setup>
  ...
  const modules = import.meta.globEager('../../assets/pa-icons/*.vue')
  for (const path in modules) {
    const name = path.match(/([A-Za-z0-9_-]+)/g)[2]
    paIcons.push(`PaIcon${name}`)
  }
