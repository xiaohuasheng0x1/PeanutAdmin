  ...
  <a-layout-content class="flex flex-col lg:h-full relative w-full">
    <div class="_crud-header flex flex-col mb-2" ref="crudHeaderRef">
      <a-tabs
          ...
          :hide-content="true"
          @change="tabChange"
          @tab-click="paEvent.customeEvent(options.tabs, $event, 'onClick')"
          class="pa-tabs mb-5"
      >
    ...
    <pa-setting ref="crudSettingRef" />

    <pa-form ref="crudFormRef" @success="requestSuccess">
      <template v-for="slot in Object.keys($slots)" #[slot]="component">
        <slot :name="slot" v-bind="component" />
      </template>
    </pa-form>

    <pa-import ref="crudImportRef" />

    <pa-context-menu ref="crudContextMenuRef" @execCommand="execContextMenuCommand" />
...
import { loadDict } from '@cps/pa-form/js/networkRequest.js'
import ColumnService from './js/columnService'

import paSearch from './components/search.vue'
import paForm from './components/form.vue'
import paSetting from './components/setting.vue'
import paImport from './components/import.vue'
import paColumn from './components/column.vue'
import paContextMenu from './components/contextMenu.vue'
...
import { paEvent } from '@cps/pa-form/js/formItemMixin.js'
...
const init = async () => {
  ...
  if (isUndefined(options.value.id)) {
    options.value.id = 'PaCrud_' + Math.floor(Math.random() * 100000 + Math.random() * 20000 + Math.random() * 5000)
  }
...
const requestHandle = async () => {
  ...
  if (isFunction(currentApi.value)) {
    ...
    console.error(`pa-crud error：crud.api not is Function.`)
  }
...
const tabChange = async (value) => {
  ...
  await paEvent.customeEvent(options.value.tabs, value, 'onChange')
  await refresh()
}
...
const execContextMenuCommand = async (args) => {
  ...
  switch(item.operation) {
    ...
    default:
      await paEvent.customeEvent(item, args, 'onCommand')
      break;
  }
}
