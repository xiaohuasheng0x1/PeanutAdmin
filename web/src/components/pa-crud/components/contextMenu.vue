<template>
  <ul
    class="pa-crud-contextmenu shadow-lg"
...
watch(
    ...
    value => {
      ...
      const crudContextMenuhandler = (e) => {
        const dom = document.querySelector('.pa-crud-contextmenu')
...
const openContextMenu = async (ev, record) => {
  ...
  await nextTick(() => {
    const domHeight = document.querySelector('.pa-crud-contextmenu').offsetHeight
