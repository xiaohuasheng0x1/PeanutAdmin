  ...
  <div class="w-full">
    <a-spin :loading="formLoading" :tip="options.loadingText" class="w-full">
      <div
        ...
        :class="['pa-form-title', options.formTitleClass]"
      ...
      <a-form
        ref="maFormRef"
        :model="form"
        :class="['pa-form', options?.customClass]"
...
for (const path in containerList) {
  ...
  const containerName = `Pa${toHump(name)}`
...
for (const path in componentList) {
  const name = path.match(/([A-Za-z0-9_-]+)/g)[1]
  const componentName = `Pa${toHump(name)}`
...
paEvent.handleCommonEvent(options.value, 'onCreated')

onMounted(async () => {
  paEvent.handleCommonEvent(options.value, 'onMounted')
  options.value.init && await init()
  paEvent.handleCommonEvent(options.value, 'onInit')
})
