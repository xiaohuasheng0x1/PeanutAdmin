...
watch(() => formModel.value[props.component.dataIndex], (value) => {
  if (isArray(value)) {
    value.forEach((data, index) => {
      ...
      paEvent.customeEvent(props.component, {formList: viewFormList.value[index], data, index}, 'onAdd')
    })
  }
},{
  immediate: true,
...
const addItem = async (data = {}) => {
  ...
  viewFormList.value[index] = cloneDeep(formList)
  paEvent.customeEvent(props.component, {formList: viewFormList.value[index], data, index: index}, 'onAdd')
  formModel.value[props.component.dataIndex].push(data)
}

const deleteItem = async (index) => {
  let res = await paEvent.customeEvent(props.component, {index}, 'onDelete')
