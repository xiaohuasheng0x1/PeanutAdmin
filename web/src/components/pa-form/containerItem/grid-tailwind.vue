...
<template>
  ...
  >
    <template v-for="(col, colIndex) in (props.component?.cols ?? [])" :key="colIndex">
      <pa-grid-tailwind-col :component="col">
        <template v-for="slot in Object.keys($slots)" #[slot]="component" >
          <slot :name="slot" v-bind="component" />
        </template>
      </pa-grid-tailwind-col>
    </template>
  </div>
</template>
...
import PaGridTailwindCol from './grid-tailwind-col.vue'
import { paEvent } from '../js/formItemMixin.js'
const props = defineProps({ component: Object })
const gridClass = ref(['pa-grid', 'grid', 'lg:grid-cols-' + props.component?.colNumber ?? 1])
