...
<template>
  <div>
    <pa-image-upload v-if="props.type === 'image'" v-model="file" />
    <pa-file-upload v-if="props.type === 'file'" v-model="file" />
    <pa-chunk-upload v-if="props.type === 'chunk'" v-model="file" />
  </div>
</template>
<script setup>
...
import PaImageUpload from './components/image-upload.vue'
import PaFileUpload from './components/file-upload.vue'
import PaChunkUpload from './components/chunk-upload.vue'
...
if (! ['id', 'url', 'hash'].includes(props.returnType)) {
  Message.error('PaUpload组件props的returnType只能为：id, url, hash 其中一个')
  console.error('PaUpload组件props的returnType只能为：id, url, hash 其中一个')
}
