    ...
    <pa-crud :options="crud" :columns="columns" ref="crudRef">
      ...
      <template #operationBeforeExtend="{ record }">
        <a-popconfirm content="确定立刻执行一次?" position="bottom" @ok="run(record)" >
          <a-link v-auth="['crontab:run']"><icon-caret-right /> 执行一次</a-link>
        </a-popconfirm>
...
<script setup>
  ...
  import crontab from '@/api/backend/crontab'
  ...
  const crud = reactive({
    ...
    operationWidth: 280,
    add: { show: true, api: crontab.save, auth: ['crontab:save'] },
    edit: { show: true, api: crontab.update, auth: ['crontab:update'] },
    delete: {
      show: true,
      api: crontab.deletes, auth: ['crontab:delete'],
    },
  })
