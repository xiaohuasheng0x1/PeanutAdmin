...
<script setup>
  ...
  import attachment from '@/api/backend/attachment'
  ...
  const download = async (record) => {
    ...
    if (storeMode !== 'LOCAL') {
      ...
      url = 'backend/downloadById?id=' + record.id
    }
  ...
  const crud = reactive({
    ...
    delete: {
      show: true,
      api: attachment.deletes, auth: ['attachment:delete'],
      realApi: attachment.realDeletes, realAuth: ['attachment:realDeletes']
    },
    recovery: { show: true, api: attachment.recoverys, auth: ['attachment:recovery']},
    isDbClickEdit: false,
  })
...
<script>
export default { name: 'attachment' }
</script>
