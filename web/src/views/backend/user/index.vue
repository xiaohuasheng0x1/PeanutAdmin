      ...
      <pa-crud :options="crud" :columns="columns" ref="crudRef">
        ...
        <template #operationAfterExtend="{ record }">
          ...
          >
            ...
            <template #content>
              <a-doption value="updateCache" v-auth="['user:cache']">更新缓存</a-doption>
              <a-doption value="setHomePage" v-auth="['user:homePage']">设置首页</a-doption>
              <a-doption value="resetPassword" v-auth="['user:initUserPassword']">重置密码</a-doption>
            </template>
          </a-dropdown>
        </template>
      </pa-crud>
...
<script setup>
  ...
  import PaTreeSlider from '@cps/pa-treeSlider/index.vue'
  import dept from '@/api/backend/dept'
  import user from '@/api/backend/user'
  import commonApi from '@/api/backend/common'
  ...
  const crud = reactive({
    ...
    operationWidth: 200,
    add: { show: true, api: user.save, auth: ['user:save'] },
    edit: { show: true, api: user.update, auth: ['user:update'] },
    delete: {
      show: true,
      api: user.deletes, auth: ['user:delete'],
      realApi: user.realDeletes, realAuth: ['user:realDeletes']
    },
    recovery: { show: true, api: user.recoverys, auth: ['user:recovery']},
    import: { show: true, url: 'backend/user/import', templateUrl: 'backend/user/downloadTemplate', auth: ['user:import'] },
    export: { show: true, url: 'backend/user/export', auth: ['user:export'] },
    formOption: {
      width: 800,
      layout: [
  ...
  const columns = reactive([
    ...
    },
    {
      ...
      dict: { url: 'backend/dept/tree' }, commonRules: [{ required: true, message: '所属部门必选' }],
    ...
    { 
      ...
      dict: { url: 'backend/role/list', props: { label: 'name', value: 'id' } }, hide: true, 
    ...
    { 
      ...
      dict: { url: 'backend/post/list', props: { label: 'name', value: 'id' } }, hide: true,
      editDefaultValue: async (record) => {
        const response = await user.read(record.id)
