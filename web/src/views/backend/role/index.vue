...
<script setup>
  ...
  import role from '@/api/backend/role'
  ...
  const crud = reactive({
    ...
    operationWidth: 300,
    add: { show: true, api: role.save, auth: ['role:save'] },
    edit: { show: true, api: role.update, auth: ['role:update'] },
    delete: {
      show: true,
      api: role.deletes, auth: ['role:delete'],
      realApi: role.realDeletes, realAuth: ['role:realDeletes']
    },
    recovery: { show: true, api: role.recoverys, auth: ['role:recovery']},
    isDbClickEdit: false,
