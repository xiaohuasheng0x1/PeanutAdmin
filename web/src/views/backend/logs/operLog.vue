...
<template>
  <div class="pa-content-block lg:flex justify-between p-4">
    <!-- CRUD 组件 -->
    <pa-crud :options="crud" :columns="columns" @row-click="openDetails" ref="crudRef"></pa-crud>
    ...
    <a-drawer v-model:visible="visible" width="600px" :footer="false">
      ...
      >
        <a-collapse-item header="请求数据" key="request">
          <pa-code-editor v-model="currentRow.request_data" is-bind :height="150" readonly />
        </a-collapse-item>
        <a-collapse-item header="响应数据" key="response">
          <pa-code-editor v-model="currentRow.response_data" is-bind :height="150" readonly />
        </a-collapse-item>
      </a-collapse>
    </a-drawer>
  </div>
...
<script setup>
  ...
  import maCodeEditor from '@cps/pa-codeEditor/index.vue'
  import operLog from '@/api/backend/operLog'
  ...
  const crud = reactive({
    ...
    delete: { show: true, api: operLog.deletes, auth: ['operLog:delete'] },
  })
