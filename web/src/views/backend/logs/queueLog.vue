    ...
    <a-drawer v-model:visible="visible" width="600px" :footer="false">
      ...
      >
        <a-collapse-item header="队列数据" key="request">
          <pa-code-editor v-model="currentRow.queue_content" is-bind :height="150" readonly />
        </a-collapse-item>
...
<script setup>
  ...
  import queueLog from '@/api/backend/queueLog'
  ...
  const crud = reactive({
    ...
    delete: { show: true, api: queueLog.deletes, auth: ['queueLog:delete'] },
  })
