...
<template>
  <div class="lg:flex justify-between">
    <div class="pa-content-block p-5 h-full lg:w-1/2">
      <a-tabs
        type="rounded"
        ...
        <template #extra>
          <a-space>
            <a-tooltip content="添加组">
              <a-button shape="round" @click="addGroupModal" type="primary" v-auth="['config:save']">
                <template #icon><icon-plus /></template>
              </a-button>
            </a-tooltip>

            <a-tooltip content="管理该组配置">
              <a-button shape="round" @click="manageConfigModal" type="primary" v-auth="['config:index']">
    ...
    <div class="pa-content-block p-5 h-full lg:w-1/2 lg:ml-3 mt-3 lg:mt-0" v-if="$common.auth(['setting:config:save'])">
...
<script setup>
  ...
  import config from '@/api/backend/config'
