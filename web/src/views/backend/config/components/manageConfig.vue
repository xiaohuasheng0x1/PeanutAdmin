...
<template>
  <a-modal fullscreen v-model:visible="visible" :footer="false">
    ...
    <div class="h-full">
      <pa-crud ref="crudRef" :options="crud" :columns="columns" />
    </div>
  </a-modal>
</template>
...
const crud = reactive({
  ...
  operationWidth: 160,
  edit: { show: true, api: config.update, auth: ['config:update'] },
  delete: { show: true, api: config.delete, auth: ['config:delete'] },
...
const columns = reactive([
  {
    ...
    formType: 'select',
    hide: true,
    dict: { url: 'backend/configGroup/index', props: { label: 'name', value: 'id' } },
