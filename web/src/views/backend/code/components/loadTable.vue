...
<template>
  <a-modal width="1100px" v-model:visible="visible" :on-before-ok="loadTable" :align-center="false" top="50px" unmount-on-close>
    ...
    <!-- CRUD 组件 -->
    <pa-crud :options="crud" :columns="columns" ref="crudRef" @selection-change="handlerSelection">
...
<script setup>
  ...
  import dataMaintain from '@/api/backend/dataMaintain'
  import dataSource from '@/api/backend/datasource'
  import generate from '@/api/backend/generate'
  import { Message } from '@arco-design/web-vue'

  const crudRef = ref()
  const selecteds = ref([])
  const visible = ref(false)
  const sourceName = ref('PeanutAdmin')
  const newName = ref({})
  ...
  const switchSource = () => {
    ...
    if (sourceName.value === 'PeanutAdmin') {
    ...
    crud.api = sourceName.value === 'PeanutAdmin' ? dataMaintain.getPageList : dataSource.getDataSourceTablePageList
    crudRef.value.requestParams['id'] = sourceName.value === 'PeanutAdmin' ? undefined : sourceName.value
