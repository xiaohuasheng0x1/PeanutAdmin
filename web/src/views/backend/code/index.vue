...
<template>
  <div class="pa-content-block lg:flex justify-between p-4">
    <!-- CRUD 组件 -->
    <pa-crud :options="crud" :columns="columns" ref="crudRef" @selection-change="selectionChange">
      <!-- 表格按钮后置扩展 -->
      <template #tableAfterButtons>
        <a-button
          v-auth="['code:generate']"
          type="outline"
          @click="batchGenerate"
        ><template #icon><icon-code /></template>生成代码</a-button>
        <a-button
          v-auth="['code:loadTable']"
...
<script setup>
  ...
  const generateCode = async (ids) => {
    ...
    if (response.message && ! response.success) {
      ...
      tool.download(response, 'Peanutadmin.zip')
  ...
  const crud = reactive({
    ...
    delete: {
      show: true,
      api: generate.deletes, auth: ['code:delete'],
    },
  })
