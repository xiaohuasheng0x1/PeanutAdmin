...
<script setup>
  ...
  const crud = reactive({
    ...
    operationWidth: 220,
    add: { show: true, api: dept.save, auth: ['dept:save'] },
    edit: { show: true, api: dept.update, auth: ['dept:update'] },
    delete: {
      show: true,
      api: dept.deletes, auth: ['dept:delete'],
      realApi: dept.realDeletes, realAuth: ['dept:realDeletes']
    },
    recovery: { show: true, api: dept.recoverys, auth: ['dept:recovery']},
    isExpand: true,
  })
