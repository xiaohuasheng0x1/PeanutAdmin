<template>
  <a-modal v-model:visible="visible" fullscreen :footer="false">
    ...
    <!-- CRUD 组件 -->
    <pa-crud :options="crud" :columns="columns" ref="crudRef" class="mt-3">
      <template #tableBeforeButtons>
        <pa-user text="新增领导" :onlyId="false" :isEcho="false" v-model="users" @success="selectedSuccess" />
      </template>
    </pa-crud>
...
<script setup>
  ...
  const crud = reactive({
    ...
    delete: {
      show: true,
      api: dept.delLeader, auth: ['dept:update'],
    },
