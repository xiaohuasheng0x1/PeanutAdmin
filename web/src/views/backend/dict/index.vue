...
<script setup>
  ...
  import { dictType } from '@/api/backend/dict'
  ...
  const crud = reactive({
    ...
    operationWidth: 240,
    add: { show: true, api: dictType.save, auth: ['dict:save'] },
    edit: { show: true, api: dictType.update, auth: ['dict:update'] },
    delete: {
      show: true,
      api: dictType.deletes, auth: ['dict:delete'],
      realApi: dictType.realDelete, realAuth: ['dict:realDeletes']
    },
    recovery: { show: true, api: dictType.recoverys, auth: ['dict:recovery']},
  })
