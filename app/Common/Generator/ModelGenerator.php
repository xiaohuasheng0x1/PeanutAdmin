 ...
 */
class ModelGenerator extends PaGenerator implements CodeGenerator
{
    /**
     * @var GenerateTables
     */
     ...
     */
    public function setGenInfo(GenerateTables $model): ModelGenerator
    {
        $this->model = $model;
    ...
    public function generator(): void
    {
        if ($this->model->generate_type === 1) {
            $path = BASE_PATH . "/runtime/generate/php/app/Backend/Model/";
        } else {
            $path = BASE_PATH . "/app/Backend/Model/";
        }
        $this->filesystem->exists($path) || $this->filesystem->makeDirectory($path, 0755, true, true);

        $command = [
            'command'  => 'pa:model-gen',
        ...
        if ($application->run($input, $output) === 0) {
            ...
            if ($modelName[strlen($modelName) - 1] == 's' && $modelName[strlen($modelName) - 2] != 's') {
                ...
                $oldPath = BASE_PATH . "/app/Backend/Model/{$oldName}.php";
                $sourcePath = BASE_PATH . "/app/Backend/Model/{$modelName}.php";
                $this->filesystem->put(
                    $sourcePath,
            ...
            // 压缩包下载
            if ($this->model->generate_type === 1) {
                $toPath = BASE_PATH . "/runtime/generate/php/app/Backend/Model/{$modelName}.php";
    ...
    {
        $data = make(GenerateColumnsService::class)->getList(
