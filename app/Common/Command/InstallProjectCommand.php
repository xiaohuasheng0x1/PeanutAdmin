 ...
 */
#[Command]
class InstallProjectCommand extends PaCommand
{
    /**
     * 安装命令
     * @var string|null
     */
    protected ?string $name = 'pa:install';
    ...
    {
        parent::configure();
        $this->setHelp('run "php bin/hyperf.php pa:install" install PeanutAdmin system');
        $this->setDescription('PeanutAdmin system install command');

        $this->addOption('option', '-o', InputOption::VALUE_OPTIONAL, 'input "-o reset" is re install PeanutAdmin');
    }
    ...
    {
        $this->line('-----------------------------------------------------------', 'comment');
        $this->line('Hello, welcome use PeanutAdmin system.', 'comment');
    ...
    protected function setDataBaseInformationAndRedis(): void
    {
        ...
        if ($dbAnswer) {
            ...
            $dbname = $this->ask('please input database name, default:', 'peanutadmin');
     ...
     * 数据迁移
     */
    protected function dataMigrate(): void
    {
        /* @var Peanut $peanut */
        $this->line("Installation of local modules is about to begin...\n", 'comment');
        $this->call('pa:migrate-run', ['name' => 'peanutAdmin', '--force' => 'true']);
        $this->call('pa:seeder-run',  ['name' => 'peanutAdmin', '--force' => 'true']);
        $this->initUserData();
    }

    protected function setOthers(): void
    {
        $this->line(PHP_EOL . ' PeanutAdmin set others items...' . PHP_EOL, 'comment');
        $this->call('pa:update');
        $this->call('pa:jwt-gen', [ '--jwtSecret' => 'JWT_SECRET' ]);
        $this->call('pa:jwt-gen', [ '--jwtSecret' => 'JWT_API_SECRET' ]);
    }
    ...
    protected function initUserData(): void
    {
        ...
        if (\Hyperf\Database\Schema\Schema::hasTable('user_dept')) {
...
PeanutAdmin Version: %s
default username: superAdmin
default password: admin123', $this->getInfo(), Peanut::getVersion()), 'comment');
    }
