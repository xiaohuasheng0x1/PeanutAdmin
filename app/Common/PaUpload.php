...
class PaUpload
{
    ...
    protected function handleUpload(UploadedFile $uploadedFile, array $config): array
    {
        ...
        $this->evDispatcher->dispatch(new \App\Common\Event\UploadAfter($fileInfo));
    ...
    public function handleSaveNetworkImage(array $data): array
    {
        ...
        try {
            ...
            if ($model = (new UploadFileMapper)->getFileInfoByHash($hash)) {
    ...
    {
        return $this->container->get(ConfigService::class)->getConfigByKey('upload_mode')['value'] ?? 1;
    }
    ...
    {
        return $this->container->get(PaRequest::class)->getScheme();
    }
}