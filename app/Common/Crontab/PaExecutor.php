...
use App\Common\Crontab\Mutex\RedisServerMutex;
use App\Common\Crontab\Mutex\RedisTaskMutex;
...
class PaExecutor
{
     ...
     * @throws \Psr\Container\NotFoundExceptionInterface
     */
    public function execute(PaCrontab $crontab, bool $run = false): ?bool
    {
        if ((! $crontab instanceof PaCrontab || ! $crontab->getExecuteTime()) && !$run) {
            return null;
        }
        $diff = 0;
    ...
    protected function runInSingleton(PaCrontab $crontab, Closure $runnable): Closure
    {
    ...
    protected function runOnOneServer(PaCrontab $crontab, Closure $runnable): Closure
    {
        return function () use ($crontab, $runnable) {
     ...
     * @return Closure
     */
    protected function decorateRunnable(PaCrontab $crontab, Closure $runnable): Closure
    {
    ...
    protected function logResult(PaCrontab $crontab, bool $isSuccess, $result = '')
    {
        if ($this->logger) {
            if ($isSuccess) {
        ...
        $logService = $this->container->get(CrontabService::class);
        $data = [
            ...
            'status' => $isSuccess ? PaModel::ENABLE : PaModel::DISABLE,
