...
trait ServiceTrait
{
    ...
    {
        return $value == PaModel::ENABLE ? $this->mapper->enable([ $id ], $filed) : $this->mapper->disable([ $id ], $filed);
    }
    ...
    {
        if (empty($dto)) {
            return container()->get(PaResponse::class)->error('导出未指定DTO');
        }
        ...
        return (new PaCollection())->export($dto, $filename, $this->mapper->getList($params), $callbackData);
    }
