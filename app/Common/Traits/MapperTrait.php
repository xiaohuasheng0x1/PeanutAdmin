...
trait MapperTrait
{
    ...
    {
        if (! config('peanutadmin.remote_api_enabled')) {
            throw new PaException('系统未启用【远程通用列表查询】', 500);
        }
        /* @var $model PaModel */
        $model = $this->getModel();
        $query = null;
        ...
        return method_exists($this, 'handleItems')
            ? ( new PaCollection($this->handleItems($query->get($params['select'] ?? ['*']))) )->toArray()
    ...
    {
        $params['_peanutadmin_tree'] = true;
        $params['_peanutadmin_tree_pid'] = $parentField;
    ...
    {
        // 对树型数据强行加个排序
        if (isset($params['_peanutadmin_tree'])) {
            $query->orderBy($params['_peanutadmin_tree_pid']);
        }
     ...
     * @return mixed
     */
    public function first(array $condition, array $column = ['*']): ?PaModel
    {
     ...
     */
    public function readByRecycle(int $id): ?PaModel
    {
     ...
     * @return PaModel
     */
    public function getModel(): PaModel
    {
    ...
    {
        return (new PaCollection())->import($dto, $this->getModel(), $closure);
    }
     ...
     * @return void
     */
    public function dynamicRelations(\App\Common\PaModel &$model, &$config): void
    {
