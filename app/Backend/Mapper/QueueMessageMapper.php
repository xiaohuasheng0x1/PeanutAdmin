 ...
 * 信息管理Mapper类
 */
class QueueMessageMapper extends AbstractMapper
{
    /**
     * @var QueueMessage
     */
    ...
    public function handleSearch(Builder $query, array $params): Builder
    {
        ...
        if (!empty($params['getReceive'])) {
            ...
            $sql = <<<sql
                id IN ( 
                    SELECT `message_id` FROM `{$prefix}queue_message_receive` WHERE `user_id` = ?
    ...
    {
        foreach ($ids as $id) {
            $result = Db::table('queue_message_receive')
                ->where('message_id', $id)
                ->where('user_id', user()->getId())
                ->value($columnName);

            if ($result != $value) {
                Db::table('queue_message_receive')
                    ->where('message_id', $id)
                    ->where('user_id', user()->getId())
